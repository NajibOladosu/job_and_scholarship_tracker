...
# Option 3: Create new venv if neither exists
    ...
    python3 -m venv /tmp/worker_venv
    source /tmp/worker_venv/bin/activate
...
exec python -m celery -A config worker --loglevel=info --concurrency=2 --max-tasks-per-child=1000
